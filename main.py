...
def initialize_qrc_solver():
    ...
    if qrc_solver is None:
        ...
        training_data = generate_synthetic_training_data(n_instances=15)
...
def optimize_routes(problem: VrpProblem):
    ...
    Supports both QAOA and QRC methods.
    """
    ...
    try:
        ...
        # Store in global state for adaptation
        ...
        if problem.use_qrc and qrc_solver and qrc_solver.trained:
            ...
            execution_time = solution.adaptation_time
            is_quantum = True
...
# ============================================================
# REAL-TIME ADAPTATION ENDPOINTS (KILLER FEATURES!)
# ============================================================
...
def handle_traffic_jam(event: TrafficJamEvent):
    ...
    QRC adapts in < 1 second without re-optimization.
    
    This is the DEMO KILLER FEATURE #1.
    """
    ...
    try:
        ...
        # Update traffic multipliers for visualization
        ...
        return {
            ...
            "notes": f"Adapted to traffic jam ({len(event.jam_locations)} affected routes, severity: {event.jam_severity}x) in {adaptation_time:.3f}s"
        }
...
def handle_priority_delivery(event: PriorityDeliveryEvent):
    ...
    QRC instantly reroutes fleet to include new stop.
    
    This is the DEMO KILLER FEATURE #2.
    """
    logger.warning(f"🚨 PRIORITY DELIVERY: Location {event.priority_location} (priority level: {event.priority_level})")
    ...
    try:
        ...
        # Validate priority location
        n_locations = current_problem_state['distance_matrix'].shape[0]
        if event.priority_location >= n_locations:
            raise ValueError(f"Invalid priority location {event.priority_location} (max: {n_locations-1})")
        
        # Adapt to priority delivery
        ...
        return {
            ...
            "notes": f"Added priority delivery at location {event.priority_location} (level {event.priority_level}) in {adaptation_time:.3f}s"
        }
...
def get_current_state():
    """
    Get current problem state including traffic conditions.
    Useful for visualization and debugging.
    """
    if current_problem_state['distance_matrix'] is None:
        return {
            "status": "no_problem",
            "message": "No active problem. Call /api/optimize first."
        }
...
def reset_state():
    """
    Reset problem state (useful for demos).
    """
    ...
    return {
        "status": "reset",
        "message": "Problem state cleared. QRC reservoir remains trained."
    }

# ============================================================
# COMPARISON ENDPOINT (Show QRC vs QAOA)
# ============================================================

@app.post("/api/compare-methods")
def compare_methods(problem: VrpProblem):
    """
    Compare QAOA vs QRC on the same problem.
    Shows speed advantage of QRC.
    """
    ...
    try:
        ...
        if qrc_solver and qrc_solver.trained:
            ...
            logger.info(f"QRC is {speedup:.1f}x faster than QAOA!")
            
            results['comparison'] = {
                'speedup': speedup,
                'winner': 'QRC' if qrc_time < qaoa_time else 'QAOA',
                'qaoa_advantage': (results['qaoa']['distance'] - results['qrc']['distance']) / results['qrc']['distance'] * 100
            }
...
if __name__ == "__main__":
    ...
    print("  • Quantum Reservoir Computing for exponential memory")
    print("  • Real-time traffic jam adaptation (< 1 second)")
    print("  • Instant priority delivery insertion")
    print("  • No re-optimization needed!")
    print("\n📡 Endpoints:")
    print("  POST /api/optimize         - Solve VRP (QAOA or QRC)")
    print("  POST /api/traffic-jam      - Real-time traffic adaptation")
    print("  POST /api/priority-delivery - Urgent delivery insertion")
    print("  POST /api/compare-methods  - QAOA vs QRC comparison")
    print("  GET  /api/current-state    - View problem state")
    print("  POST /api/reset            - Reset problem")
    print("\n" + "=" * 80 + "\n")