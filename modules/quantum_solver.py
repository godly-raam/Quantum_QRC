...
def solve_quantum_vrp(
    ...
    try:
        ...
        if estimated_qubits <= 12:
            ...
            sampler = SamplerV2(default_shots=4096, seed=42)
            optimizer = COBYLA(maxiter=150)
            adjusted_reps = min(reps, 5)
            method_note = "sampling (4096 shots)"
            ...
            sampler = SamplerV2(default_shots=2048, seed=42)
            ...
            sampler = SamplerV2(default_shots=1024, seed=42)
        ...
        try:
            ...
            # Qiskit Optimization 0.7 returns ordered [source, target] edge
            # lists per vehicle. Convert complete depot-to-depot chains to node routes.
            formatted_routes: List[List[int]] = []
            visited_customers = []
            for route_edges in raw_routes:
                if not route_edges:
                    continue
                route = [depot_node]
                ...
                for edge in route_edges:
                    if len(edge) != 2:
                        raise ValueError(f"Invalid QAOA route edge: {edge}")
                    source, target = int(edge[0]), int(edge[1])
                    if source != current_node:
                        raise ValueError("QAOA route edges are not contiguous")
                    route.append(target)
                    current_node = target
                    if target == depot_node:
                        break
                if route[-1] != depot_node:
                    raise ValueError("QAOA route does not return to the depot")
                formatted_routes.append(route)
                visited_customers.extend(route[1:-1])

            expected_customers = list(range(num_locations))
            expected_customers.remove(depot_node)
            if sorted(visited_customers) != expected_customers:
                raise ValueError("QAOA routes do not visit every customer exactly once")
